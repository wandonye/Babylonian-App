...
//---------------------------------------------------------------------------------------------------
void SendPushNotification0(NSString *transId, NSString *text)
//---------------------------------------------------------------------------------------------------
{
	...
	[query observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
	{
		if (snapshot.value != [NSNull null])
		{
			...
			if (recent != nil)
			{
				SendPushNotification2(recent[@"members"], DecryptText(recent[@"transId"], text));
			}
		}
	}];
}
...
//---------------------------------------------------------------------------------------------------
{
	...
	[push setData:@{@"alert":message, @"sound":@"default", @"category":@"ACTIONABLE", @"badge":@"Increment"}];
