...
//----------This is never used for translator side------
NSString* StartTranslate(PFUser *user, PFUser *translator, NSString *groupId)
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------------
    CreateTrans(userId2, groupId, members, user[PF_USER_FULLNAME], userId1, @"private");
    //---------------------------------------------------------------------------------------------------------------------------------------------
    return groupId;
}

//----------This is never used for translator side------
void RestartTranslate(NSDictionary *recent)
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    [query observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
     {
         ...
         if (snapshot.value != [NSNull null])
         {
             ...
             {
                 if ([recent[@"userId"] isEqualToString:transId]) create = NO;
             }
         }
         //-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
{
    ...
    //---------------------------------------------------------------------------------------------------
    NSDictionary *recent = @{@"recentId":recentId, @"userId":userId, @"transId":transId, @"members":members, @"description":description,
...
{
	Firebase *firebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@/Trans", FIREBASE]];
	FQuery *query = [[firebase queryOrderedByChild:@"transId"] queryEqualToValue:groupId];
	[query observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
	{
		if (snapshot.value != [NSNull null])
		{
