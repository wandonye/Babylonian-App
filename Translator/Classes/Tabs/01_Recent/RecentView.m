...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
	{
		...
		//-----------------------------------------------------------------------------------------------------------------------------------------
		[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loadTrans) name:NOTIFICATION_APP_STARTED object:nil];
		[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loadTrans) name:NOTIFICATION_USER_LOGGED_IN object:nil];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    if (([PFUser currentUser] == nil) || (firebase != nil)) return;
    //---------------------------------------------------------------------------------------------------------------------------------------------
    firebase = [[Firebase alloc] initWithUrl:[NSString stringWithFormat:@"%@/Trans", FIREBASE]];
    FQuery *query = [[firebase queryOrderedByChild:@"userId"] queryEqualToValue:[PFUser currentId]];
    [query observeEventType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot)
     {
         [self.recents removeAllObjects];
...
//-----------------------------------------------------------------------------------------------------------
{
	int total = 0;
	//-------------------------------------------------------------------------------------------------------
	for (PFObject *recent in self.recents)
	{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    TranslateViewController *translateView = [[TranslateViewController alloc]initWith:groupId];
    //TViewController *translateView = [[TViewController alloc]initWithNibName:@"TViewController" bundle:nil groupId:groupId];
